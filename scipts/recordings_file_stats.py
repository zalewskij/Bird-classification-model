...
df = pd.DataFrame(columns=['id', 'Latin name', 'length', 'fragments50', 'fragments70', 'fragments90'])
for i in range(len(train_ds)):
    ...
    n_fragments50 = len(get_thresholded_fragments(waveform, SAMPLE_RATE, parameters['n_fft'], parameters['hop_length'],
                                                  parameters['sample_length'], threshold=0.5))
    n_fragments70 = len(get_thresholded_fragments(waveform, SAMPLE_RATE, parameters['n_fft'], parameters['hop_length'],
                                                  parameters['sample_length'], threshold=0.7))
    n_fragments90 = len(get_thresholded_fragments(waveform, SAMPLE_RATE, parameters['n_fft'], parameters['hop_length'],
                                                  parameters['sample_length'], threshold=0.9))

    new_row = {'id': id, 'Latin name': latin_name, 'length': length,
               'fragments50': int(n_fragments50), 'fragments70': int(n_fragments70), 'fragments90': int(n_fragments90)}
    df = df._append(new_row, ignore_index=True)

df.to_csv("train_recording_statistics.csv")
